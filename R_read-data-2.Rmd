...
```{r}
library(tidyverse)
data_fev<- read_csv("RS-Praperation/fev.csv")
```
library(here)
...
library(readr)    
library(magrittr)  
fev_data <-read_csv("RS-Praperation/fev.csv")%>%
filter(age > 20)
```
```{r}
head(fev_data)
...
```{r}
library(readxl)
txt.data <- read.table("RS-Praperation/psa.txt")
...
```{r}
library(here)
summary("RS-Praperation/data_fev$column")
```
...
```{r}
max(fev_data$fev, na.rm = TRUE)
mean(fev_data$fev, na.rm = TRUE)
sd(fev_data$fev, na.rm = TRUE)
```
